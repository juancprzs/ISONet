...
files = sorted(glob(pattern), key=key)

print('These files will be read: ', files)
...
for f in tqdm(files):
    ...
    # accuracies
    accs = [float(x.split('|')[1].split(':')[1].strip()) for x in lines]
    # xent loss
    xent = [float(x.split('|')[2].split(':')[1].strip()) for x in lines]
    # orthogonality loss
    orth = [float(x.split('|')[3].split(':')[1].strip()) for x in lines]
    # Lipschitz constants
    lips = [float(x.split('|')[5].split(':')[1].strip()) for x in lines]
    # plot stuff
